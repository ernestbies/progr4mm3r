import {
    AwardIcon, BottomItem, CompetitionIcon,
    CompetitionInfo,
    CompetitionSubtitle,
    CompetitionTitle, IconTechWrapper, IconWrapper, Line, LinksBlock, ProjectDesc,
    ProjectImage, ProjectImageWrapper,
    ProjectInfo,
    ProjectTitle,
    StyledCompetitionItem, StyledIcon
    ...
    const renderLinksBlock = () => {
        return links.map((e, index) => {
                return e.name === 'Github' ?
                    <IconWrapper key={index} href={e.url} target={'_blank'} rel={'noreferrer'} color={e.color}>
