...
import {Sign, StyledDiv, StyledText, Subtitle, BadgeWrapper} from "./SectionBadge.styles";
import PropTypes from 'prop-types';
import {Logo} from "../Logo/Logo";

const SectionBadge = ({content, subtitle, specialNumber, backgroundColor, fontColor, signFontColor, signColor, position, lightMode, style, absolute, onClick}) => {

    const SPECIAL_SIGN_TEXT = 'EB_0';

    return (
        <BadgeWrapper style={style} absolute={absolute} position={position}>
            <StyledDiv onClick={onClick} position={position} backgroundColor={backgroundColor} width={content.length}>
